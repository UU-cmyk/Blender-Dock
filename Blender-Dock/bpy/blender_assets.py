...
def modify_asset_lib(identifier: str, new_name=None, new_path=None) -> None:
    ...
    bpy.ops.wm.save_userpref()
    print(f"修改后: {target.name} -> {target.path}")
...
def main() -> None:
    ...
    if not args:
        ...
        print("  add <名称> <路径>")
        print("  remove <名称|索引>")
        print("  modify <名称|索引> --name <新名称> --path <新路径>")
        return
